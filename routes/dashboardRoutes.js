...
router.get('/adm2', checkUserAndProfissional, async (req, res) => {
  try {
    if (!req.user || !req.user.profissionalId) {
      return res.status(401).send('Usuário não autenticado ou sem ID de profissional');
    }
...
router.get('/admGestor', checkUserAndProfissional, async (req, res) => {
  try {
    if (!req.user || !req.user.profissionalId) {
      return res.status(401).send('Usuário não autenticado ou sem ID de profissional');
    }
    ...
    res.render('dashboard/admGestor', {
      user: req.user,
      cargo: req.profissional.cargo,
      mensagensNaoLidas,  
      notificacoesNaoLidas,  
